/*
 * This file is generated by jOOQ.
*/
package kr.pe.sinnori.impl.jooq.tables.records;


import java.sql.Timestamp;

import javax.annotation.Generated;

import kr.pe.sinnori.impl.jooq.tables.SbMemberTb;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record11;
import org.jooq.Row11;
import org.jooq.impl.UpdatableRecordImpl;
import org.jooq.types.UByte;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.10.6"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class SbMemberTbRecord extends UpdatableRecordImpl<SbMemberTbRecord> implements Record11<String, String, String, String, Byte, Byte, String, String, UByte, Timestamp, Timestamp> {

    private static final long serialVersionUID = 316623228;

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.user_id</code>. 사용자 아이디
     */
    public void setUserId(String value) {
        set(0, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.user_id</code>. 사용자 아이디
     */
    public String getUserId() {
        return (String) get(0);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.nickname</code>. 별명
     */
    public void setNickname(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.nickname</code>. 별명
     */
    public String getNickname() {
        return (String) get(1);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.pwd_base64</code>. 비밀번호, 비밀번호는 해쉬 값으로 변환되어 base64 형태로 저장된다.
     */
    public void setPwdBase64(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.pwd_base64</code>. 비밀번호, 비밀번호는 해쉬 값으로 변환되어 base64 형태로 저장된다.
     */
    public String getPwdBase64() {
        return (String) get(2);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.pwd_salt_base64</code>. 비밀번호를 해쉬로 바꿀때 역 추적 방해를 목적으로 함께 사용하는 랜덤 값
     */
    public void setPwdSaltBase64(String value) {
        set(3, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.pwd_salt_base64</code>. 비밀번호를 해쉬로 바꿀때 역 추적 방해를 목적으로 함께 사용하는 랜덤 값
     */
    public String getPwdSaltBase64() {
        return (String) get(3);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.member_gb</code>. 회원 구분, 0:관리자, 1:일반회원
     */
    public void setMemberGb(Byte value) {
        set(4, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.member_gb</code>. 회원 구분, 0:관리자, 1:일반회원
     */
    public Byte getMemberGb() {
        return (Byte) get(4);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.member_st</code>. 회원 상태, 0:정상, 1:블락, 2:탈퇴
     */
    public void setMemberSt(Byte value) {
        set(5, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.member_st</code>. 회원 상태, 0:정상, 1:블락, 2:탈퇴
     */
    public Byte getMemberSt() {
        return (Byte) get(5);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.pwd_hint</code>. 비밀번호 힌트, 비밀번호 분실시 답변 유도용 사용자한테 보여주는 힌트
     */
    public void setPwdHint(String value) {
        set(6, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.pwd_hint</code>. 비밀번호 힌트, 비밀번호 분실시 답변 유도용 사용자한테 보여주는 힌트
     */
    public String getPwdHint() {
        return (String) get(6);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.pwd_answer</code>. 비밀번호 답변, 비밀번호 분실시 맞춘다면 비밀번호 재 설정 혹은 비밀번호 초기화를 진행한다.
     */
    public void setPwdAnswer(String value) {
        set(7, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.pwd_answer</code>. 비밀번호 답변, 비밀번호 분실시 맞춘다면 비밀번호 재 설정 혹은 비밀번호 초기화를 진행한다.
     */
    public String getPwdAnswer() {
        return (String) get(7);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.pwd_fail_cnt</code>. 비밀번호 틀린 횟수, 로그인시 비밀번호 틀릴 경우 1 씩 증가하며 최대 n 번까지 시도 가능하다.  비밀번호를 맞쳤을 경우 0 으로 초기화 된다.
     */
    public void setPwdFailCnt(UByte value) {
        set(8, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.pwd_fail_cnt</code>. 비밀번호 틀린 횟수, 로그인시 비밀번호 틀릴 경우 1 씩 증가하며 최대 n 번까지 시도 가능하다.  비밀번호를 맞쳤을 경우 0 으로 초기화 된다.
     */
    public UByte getPwdFailCnt() {
        return (UByte) get(8);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.reg_dt</code>. 회원 가입일
     */
    public void setRegDt(Timestamp value) {
        set(9, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.reg_dt</code>. 회원 가입일
     */
    public Timestamp getRegDt() {
        return (Timestamp) get(9);
    }

    /**
     * Setter for <code>SB_DB.SB_MEMBER_TB.mod_dt</code>. 회원 정보 수정일
     */
    public void setModDt(Timestamp value) {
        set(10, value);
    }

    /**
     * Getter for <code>SB_DB.SB_MEMBER_TB.mod_dt</code>. 회원 정보 수정일
     */
    public Timestamp getModDt() {
        return (Timestamp) get(10);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Record1<String> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record11 type implementation
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Row11<String, String, String, String, Byte, Byte, String, String, UByte, Timestamp, Timestamp> fieldsRow() {
        return (Row11) super.fieldsRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Row11<String, String, String, String, Byte, Byte, String, String, UByte, Timestamp, Timestamp> valuesRow() {
        return (Row11) super.valuesRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field1() {
        return SbMemberTb.SB_MEMBER_TB.USER_ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field2() {
        return SbMemberTb.SB_MEMBER_TB.NICKNAME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field3() {
        return SbMemberTb.SB_MEMBER_TB.PWD_BASE64;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field4() {
        return SbMemberTb.SB_MEMBER_TB.PWD_SALT_BASE64;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Byte> field5() {
        return SbMemberTb.SB_MEMBER_TB.MEMBER_GB;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Byte> field6() {
        return SbMemberTb.SB_MEMBER_TB.MEMBER_ST;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field7() {
        return SbMemberTb.SB_MEMBER_TB.PWD_HINT;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field8() {
        return SbMemberTb.SB_MEMBER_TB.PWD_ANSWER;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<UByte> field9() {
        return SbMemberTb.SB_MEMBER_TB.PWD_FAIL_CNT;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field10() {
        return SbMemberTb.SB_MEMBER_TB.REG_DT;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field11() {
        return SbMemberTb.SB_MEMBER_TB.MOD_DT;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component1() {
        return getUserId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component2() {
        return getNickname();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component3() {
        return getPwdBase64();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component4() {
        return getPwdSaltBase64();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte component5() {
        return getMemberGb();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte component6() {
        return getMemberSt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component7() {
        return getPwdHint();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component8() {
        return getPwdAnswer();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UByte component9() {
        return getPwdFailCnt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component10() {
        return getRegDt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component11() {
        return getModDt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value1() {
        return getUserId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value2() {
        return getNickname();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value3() {
        return getPwdBase64();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value4() {
        return getPwdSaltBase64();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte value5() {
        return getMemberGb();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte value6() {
        return getMemberSt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value7() {
        return getPwdHint();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value8() {
        return getPwdAnswer();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UByte value9() {
        return getPwdFailCnt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value10() {
        return getRegDt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value11() {
        return getModDt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value1(String value) {
        setUserId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value2(String value) {
        setNickname(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value3(String value) {
        setPwdBase64(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value4(String value) {
        setPwdSaltBase64(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value5(Byte value) {
        setMemberGb(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value6(Byte value) {
        setMemberSt(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value7(String value) {
        setPwdHint(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value8(String value) {
        setPwdAnswer(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value9(UByte value) {
        setPwdFailCnt(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value10(Timestamp value) {
        setRegDt(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord value11(Timestamp value) {
        setModDt(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbMemberTbRecord values(String value1, String value2, String value3, String value4, Byte value5, Byte value6, String value7, String value8, UByte value9, Timestamp value10, Timestamp value11) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        value11(value11);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached SbMemberTbRecord
     */
    public SbMemberTbRecord() {
        super(SbMemberTb.SB_MEMBER_TB);
    }

    /**
     * Create a detached, initialised SbMemberTbRecord
     */
    public SbMemberTbRecord(String userId, String nickname, String pwdBase64, String pwdSaltBase64, Byte memberGb, Byte memberSt, String pwdHint, String pwdAnswer, UByte pwdFailCnt, Timestamp regDt, Timestamp modDt) {
        super(SbMemberTb.SB_MEMBER_TB);

        set(0, userId);
        set(1, nickname);
        set(2, pwdBase64);
        set(3, pwdSaltBase64);
        set(4, memberGb);
        set(5, memberSt);
        set(6, pwdHint);
        set(7, pwdAnswer);
        set(8, pwdFailCnt);
        set(9, regDt);
        set(10, modDt);
    }
}
