...
public final class BoardUploadFileResEncoder extends AbstractMessageEncoder {
	...
	private void encodeBody(BoardUploadFileRes boardUploadFileRes, SingleItemEncoderIF singleItemEncoder, Object middleWritableObject) throws Exception {
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "attachFileCnt"
			...
			, boardUploadFileRes.getAttachFileCnt() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);

		java.util.List<BoardUploadFileRes.AttachFile> attachFile$2List = boardUploadFileRes.getAttachFileList();

		/** 배열 정보와 배열 크기 일치 검사 */
		if (null == attachFile$2List) {
			/** 배열 크기 지정 방식이 간접일 경우 참조하는 변수값이 0 일 경우만 배열 값으로 null 을 허용한다. */
			if (0 != boardUploadFileRes.getAttachFileCnt()) {
				String errorMessage = new StringBuilder("the var attachFile$2List is null but the value referenced by the array size[boardUploadFileRes.getAttachFileCnt()][").append(boardUploadFileRes.getAttachFileCnt()).append("] is not zero").toString();
			...
			if (boardUploadFileRes.getAttachFileCnt() != attachFile$2ListSize) {
				String errorMessage = new StringBuilder("the var attachFile$2ListSize[").append(attachFile$2ListSize).append("] is not same to the value referenced by the array size[boardUploadFileRes.getAttachFileCnt()][").append(boardUploadFileRes.getAttachFileCnt()).append("]").toString();
				throw new kr.pe.sinnori.common.exception.BodyFormatException(errorMessage);
			}
			...
			for (int i2=0; i2 < attachFile$2ListSize; i2++) {
				...
				BoardUploadFileRes.AttachFile attachFile$2 = attachFile$2List.get(i2);
