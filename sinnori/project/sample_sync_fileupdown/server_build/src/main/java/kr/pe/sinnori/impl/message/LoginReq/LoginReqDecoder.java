...
public final class LoginReqDecoder extends AbstractMessageDecoder {
	...
	/**
	 * <pre>
	 *  "단일항목 디코더"를 이용하여 "중간 다리 역활 읽기 객체" 에서 추출된 LoginReq 메시지를 반환한다.
	 * </pre>
	 * @param singleItemDecoder 단일항목 디코더
	 * @param charsetOfProject 프로젝트 문자셋
	 * @param middleReadObj 중간 다리 역활 읽기 객체
	 * @return "단일항목 디코더"를 이용하여 "중간 다리 역활 읽기 객체" 에서 추출된 LoginReq 메시지
	...
	protected AbstractMessage decodeBody(SingleItemDecoderIF singleItemDecoder, Charset charsetOfProject, Object  middleReadObj) throws OutOfMemoryError, BodyFormatException {
		...
		String sigleItemPath0 = "LoginReq";

		loginReq.setIdEncryptedBytes((byte[])
		singleItemDecoder.getValueFromMiddleReadObj(sigleItemPath0
		, "idEncryptedBytes" // itemName
		, 13 // itemTypeID
		...
		loginReq.setPwdEncryptedBytes((byte[])
		singleItemDecoder.getValueFromMiddleReadObj(sigleItemPath0
		, "pwdEncryptedBytes" // itemName
		, 13 // itemTypeID
		, "si variable length byte[]" // itemTypeName
		, -1 // itemSize
		, null // itemCharset,
		...
		loginReq.setSessionKeyBytes((byte[])
		singleItemDecoder.getValueFromMiddleReadObj(sigleItemPath0
		, "sessionKeyBytes" // itemName
		, 13 // itemTypeID
		, "si variable length byte[]" // itemTypeName
		, -1 // itemSize
		, null // itemCharset,
