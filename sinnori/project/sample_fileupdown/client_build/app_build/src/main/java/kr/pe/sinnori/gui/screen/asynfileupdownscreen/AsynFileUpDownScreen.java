...
package kr.pe.sinnori.gui.screen.asynfileupdownscreen;
...
import kr.pe.sinnori.gui.screen.asynfileupdownscreen.action.AsynDownloadSwingAction;
import kr.pe.sinnori.gui.screen.asynfileupdownscreen.action.AsynUploadSwingAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.FileUpDownScreenIF;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.LocalDriverChangeAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.LocalParentSwingAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.LocalReloadSwingAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.LocalTreeMouseListener;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.RemoteDriverChangeAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.RemoteParentSwingAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.RemoteReloadSwingAction;
import kr.pe.sinnori.gui.screen.commonfileupdown.action.RemoteTreeMouseListener;
...
public class AsynFileUpDownScreen extends JPanel implements FileUpDownScreenIF {
	private Logger log = LoggerFactory.getLogger(AsynFileUpDownScreen.class);
	 ...
	 * Create the panel.
	 */
	public AsynFileUpDownScreen(final JFrame mainFrame, MainControllerIF mainController) {
		...
		AsynUploadSwingAction uploadAction = new AsynUploadSwingAction(this.mainFrame, mainController, this, localTree, localRootNode, remoteTree, remoteRootNode, remotePathSeperator);
		uploadAction.putValue(AbstractAction.NAME, uploadButton.getText());
		uploadButton.setAction(uploadAction);
		
		AsynDownloadSwingAction downloadAction = new AsynDownloadSwingAction(this.mainFrame, mainController, this, localTree, localRootNode, remoteTree, remoteRootNode, remotePathSeperator);
	...
	public void makeRemoteTreeNode(FileListResult fileListOutObj,
			RemoteFileTreeNode remoteParentNode) {
		// String requestDirectory = (String) fileListOutObj.getAttribute("requestDirectory");
		String requestDirectory = fileListOutObj.getRequestDirectory();
		
		if (null == remotePathSeperator) {
			// remotePathSeperator = (String) fileListOutObj.getAttribute("pathSeperator");
			remotePathSeperator = fileListOutObj.getPathSeperator();
		}
		...
		int cntOfDriver = fileListOutObj.getCntOfDriver();
		if (0 == cntOfDriver) {
