...
/**
 * SyncUpFileInfo 클라이언트 코덱
 * @author Won Jonghoon
 *
 */
public final class SyncUpFileInfoClientCodec implements MessageCodecIF {

	@Override
	public AbstractMessageDecoder getMessageDecoder() throws DynamicClassCallException {
		throw new DynamicClassCallException("SyncUpFileInfo 메시지는 서버에서 클라이언트로 전달하지 않는 메시지 입니다.");
	}

	@Override
	public AbstractMessageEncoder getMessageEncoder() throws DynamicClassCallException {
		return new SyncUpFileInfoEncoder();
	}
}