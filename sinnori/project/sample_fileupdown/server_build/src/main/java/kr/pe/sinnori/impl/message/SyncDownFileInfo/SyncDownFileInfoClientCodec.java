...
/**
 * SyncDownFileInfo 클라이언트 코덱
 * @author Won Jonghoon
 *
 */
public final class SyncDownFileInfoClientCodec implements MessageCodecIF {

	@Override
	public MessageDecoder getMessageDecoder() throws DynamicClassCallException {
		throw new DynamicClassCallException("SyncDownFileInfo 메시지는 서버에서 클라이언트로 전달하지 않는 메시지 입니다.");
	}

	@Override
	public MessageEncoder getMessageEncoder() throws DynamicClassCallException {
		return new SyncDownFileInfoEncoder();
	}
}