...
public class ShareAsynConnection extends AbstractAsynConnection {
	...
	public ShareAsynConnection(String projectName, int index, 
		...
		super(projectName, index, hostOfProject, portOfProject, charsetOfProject, 
				...
				asynOutputMessageQueue, inputMessageQueue, messageProtocol,
				outputMessageReaderPool, dataPacketBufferQueueManager, clientObjectCacheManager);
		
		log.info(String.format("create MultiNoneBlockConnection, projectName=[%s%02d], mailBoxCnt=[%d]",
				projectName, index, mailBoxCnt));
		...
		try {
			...
			String errorMessage = String.format("project[%s] ShareAsynConnection[%d], fail to config a socket channel", projectName, index);
		...
		if (whetherToAutoConnect) {
			try {
				...
				log.warn(String.format("project[%s] ShareAsynConnection[%d] fail to connect server", projectName, index), e);
				// System.exit(1);
			}
		}
			...
			if (!result) {
				String errorMsg = String
						.format("서버 공용 출력 메시지 큐 응답 시간[%d]이 초과되었습니다. serverConnection=[%s], receivedLetter=[%s]",
								socketTimeOut, this.getSimpleConnectionInfo(), receivedLetter.toString());
