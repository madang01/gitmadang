	...
	public boolean isValid(Properties sourceProperties,
		...
		if (null == dependentSourceItemValue) {
			String errorMessage = new StringBuilder(
					...
					.append(dependentSourceItemKey).append("]) does not exist at the paramerter sourceProperties where the sinnori config file was loaded").toString();
			throw new IllegalArgumentException(errorMessage);
		}
		...
		if (null == dependentTargetItemValue) {
			String errorMessage = new StringBuilder(
					...
					.append(dependentTargetItemKey).append("]) does not exist at the paramerter sourceProperties where the sinnori config file was loaded").toString();
			throw new IllegalArgumentException(errorMessage);
		}
