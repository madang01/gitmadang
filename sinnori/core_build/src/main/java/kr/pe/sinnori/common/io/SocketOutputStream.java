...
public class SocketOutputStream {
	...
	private List<WrapBuffer> dataPacketBufferList = new  LinkedList<WrapBuffer>();
	/** 메시지를 추출시 생기는 부가 정보를  */
	 ...
	 */
	public SocketOutputStream(int dataPacketBufferMaxCntPerMessage, DataPacketBufferPoolManagerIF dataPacketBufferQueueManager) throws NoMoreDataPacketBufferException {
		// FIXME!
		// log.info("call");
	...
	public void beInitializedForRecycling() throws NoMoreDataPacketBufferException {
		while(dataPacketBufferList.size() > 1) {
			dataPacketBufferQueueManager.putDataPacketBuffer(dataPacketBufferList.get(0));
	 ...
	 */
	public SocketOutputStream(int dataPacketBufferMaxCntPerMessage, ArrayList<WrapBuffer> messageReadWrapBufferList,
	...
	public void destory() {
		...
		while (dataPacketBufferList.size() > 0) {
