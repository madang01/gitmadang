...
public class InputMessageReaderPoolTest extends AbstractJunitTest {
	...
	public void testAddTask_greaterThanTheParameterPoolMaxSize() {
		...
		try {
			...
			String errorMessage = e.getMessage();
			String exepecedErrorMessage = String.format("can't add any more tasks becase the number of %s InputMessageReaderPool's tasks reached the maximum[%d] number", projectName, poolMaxSize);
