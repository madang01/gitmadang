...
public class MainServerManagerTest extends AbstractJunitTest {
	...
	public void test3() {
		...
		try {
			...
			SocketChannel serverSC = SocketChannel.open();
			SelectableChannel serverSelectableChannel = serverSC.configureBlocking(true);
	...
	public void test2() {
		...
		AnyProjectConnectionPool anyProjectConnectionPoolForTest = null;
		
		try {
			anyProjectServerForTest = new AnyProjectServer(projectPartConfigurationForTest);
			anyProjectConnectionPoolForTest = new AnyProjectConnectionPool(projectPartConfigurationForTest);
			...
			emptyRes = anyProjectConnectionPoolForTest.sendSyncInputMessage(emptyReq);
	...
	private ProjectPartConfiguration getMainProjectPartConfiguration(String projectName, MessageProtocolType messageProtocolTypeForTest, 
			ConnectionType connectionTypeForTest)
			throws SinnoriConfigurationException {
		ProjectPartConfiguration projectPartConfigurationForTest = new ProjectPartConfiguration(ProjectType.MAIN, projectName);
