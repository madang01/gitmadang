...
import kr.pe.sinnori.gui.ConnectionScreen;
import kr.pe.sinnori.gui.FileTranferProcessDialog;
import kr.pe.sinnori.gui.FileTransferTaskIF;
import kr.pe.sinnori.gui.FileUpDown2Screen;
 ...
 *
 */
public class TestFtpClientCExtor extends AbstractClientExecutor implements MainControllerIF {
	private JFrame mainFrame = null;

	private ConnectionScreen connectionScreen = null;
	private FileUpDown2Screen fileUpDownScreen = null;
	...
	public void loginOK() {
		fileUpDownScreen = new FileUpDown2Screen(mainFrame, this);
	...
	public byte[] connectServer(String host, int port) {
		...
		try {
			letterFromServer = clientProject.sendInputMessage(binaryPublicKeyInObj);
			...
			log.info("ServerNotReadyException", e);
			JOptionPane.showMessageDialog(mainFrame, "서버가 준비되지 않았습니다. 서버및 네트워크 연결 상태를 점검하세요.");
			return null;
		} catch (NoMoreDataPacketBufferException e) {
			log.info("NoMoreDataPacketBufferException", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		} catch (BodyFormatException e) {
			log.info("BodyFormatException", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		} catch (MessageInfoNotFoundException e) {
			log.info("MessageInfoNotFoundException", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		}
		...
		try {
			...
			log.info("DynamicClassCallException", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		} catch (NoMoreDataPacketBufferException e) {
			log.info("NoMoreDataPacketBufferException", e);
			...
			log.info("NoMatchOutputMessage", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		} catch(MessageItemException e) {
			log.info("MessageItemException", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		} catch (ServerExcecutorUnknownException e) {
			log.info("ServerExcecutorUnknownException", e);
			JOptionPane.showMessageDialog(mainFrame, e.getMessage());
			return null;
		}
		
		// ClientSessionKeyManager clientSessionKeyManager = null;
	...
	public boolean login(String id, String pwd) {
		...
		LetterFromServer letterFromServer = null;
		try {
			letterFromServer = clientProject.sendInputMessage(loginInObj);
	...
	public OutputMessage getRemoteFileList(String requestDirectory) {
		...
		LetterFromServer letterFromServer = null;
		try {
			letterFromServer = clientProject.sendInputMessage(fileListInObj);
	...
	public OutputMessage readyUploadFile(String localFilePathName, String localFileName, long localFileSize, 
		...
		try {
			letterFromServer = clientProject.sendInputMessage(inObj);
