...
import kr.pe.sinnori.gui.action.fileupdownscreen.DownloadSwingAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.LocalDriverChangeAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.LocalParentSwingAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.LocalReloadSwingAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.LocalTreeMouseListener;
import kr.pe.sinnori.gui.action.fileupdownscreen.RemoteDriverChangeAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.RemoteParentSwingAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.RemoteReloadSwingAction;
import kr.pe.sinnori.gui.action.fileupdownscreen.RemoteTreeMouseListener;
import kr.pe.sinnori.gui.action.fileupdownscreen.UploadSwingAction;
...
public class FileUpDown2Screen extends JPanel implements FileUpDownScreenIF {
	 ...
	 * Create the panel.
	 */
	public FileUpDown2Screen(final JFrame mainFrame, MainControllerIF mainController) {
		...
		*/
		...
		UploadSwingAction uploadAction = new UploadSwingAction(this.mainFrame, mainController, this, localTree, localRootNode, remoteTree, remoteRootNode, remotePathSeperator);
	...
	public void makeLocalTreeNode(LocalFileTreeNode localWorkNode) {
		...
		File localWorkFile = localWorkNode.getFileObj();
		
		File[] subFiles = localWorkFile.listFiles();
		...
		*/
		...
		for (int i = 0; i < subFiles.length; i++) {
			...
			LocalFileTreeNode localChildWorkNode = null;
			
			if (workFile.isDirectory()) {
				localChildWorkNode = new LocalFileTreeNode(workFile,
			...
			localWorkNode.add(localChildWorkNode);
		}
	}
	
	public void makeRemoteTreeNode(OutputMessage fileListOutObj,
			RemoteFileTreeNode remoteRootNode) throws MessageItemException {
		...
		remoteRootNode.setFileName(requestDirectory);
		...
		for (int i = 0; i < cntOfFile; i++) {
			...
			RemoteFileTreeNode remoteWorkNode = new RemoteFileTreeNode(fileName, fileSize,
					fileType);
			remoteRootNode.add(remoteWorkNode);
		}
	}
}
