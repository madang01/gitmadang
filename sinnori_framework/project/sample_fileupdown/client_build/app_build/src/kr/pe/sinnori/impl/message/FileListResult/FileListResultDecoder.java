...
public final class FileListResultDecoder extends MessageDecoder {
	...
	protected AbstractMessage decodeBody(SingleItemDecoderIF singleItemDecoder, Charset charsetOfProject, Object  middleReadObj) throws OutOfMemoryError, BodyFormatException {
		...
		Object driverListMiddleReadArray = singleItemDecoder.getArrayObjFromMiddleReadObj(sigleItemPath0, "DriverList", fileListResult.getCntOfDriver(), middleReadObj);
		FileListResult.DriverList[] driverListList = new FileListResult.DriverList[fileListResult.getCntOfDriver()];
		for (int i=0; i < driverListList.length; i++) {
			String sigleItemPath1 = new StringBuilder(sigleItemPath0).append(".").append("DriverList[").append(i).append("]").toString();
			Object driverListMiddleReadObj = singleItemDecoder.getMiddleReadObjFromArrayObj(sigleItemPath1, driverListMiddleReadArray, i);

			driverListList[i].setDriverName((String)
		...
		Object fileListMiddleReadArray = singleItemDecoder.getArrayObjFromMiddleReadObj(sigleItemPath0, "FileList", fileListResult.getCntOfFile(), middleReadObj);
		FileListResult.FileList[] fileListList = new FileListResult.FileList[fileListResult.getCntOfFile()];
		for (int i=0; i < fileListList.length; i++) {
			String sigleItemPath1 = new StringBuilder(sigleItemPath0).append(".").append("FileList[").append(i).append("]").toString();
			Object fileListMiddleReadObj = singleItemDecoder.getMiddleReadObjFromArrayObj(sigleItemPath1, fileListMiddleReadArray, i);

			fileListList[i].setFileName((String)
			singleItemDecoder.getValueFromMiddleReadObj(sigleItemPath1
			, "fileName" // itemName
			, 9 // itemTypeID
			, "si pascal string" // itemTypeName
			...
			, null // itemCharset,
			, charsetOfProject
			, fileListMiddleReadObj));

			fileListList[i].setFileSize((Long)
			singleItemDecoder.getValueFromMiddleReadObj(sigleItemPath1
			, "fileSize" // itemName
			, 6 // itemTypeID
			, "long" // itemTypeName
			...
			fileListList[i].setFileType((Byte)
			singleItemDecoder.getValueFromMiddleReadObj(sigleItemPath1
			, "fileType" // itemName
			, 0 // itemTypeID
			, "byte" // itemTypeName
