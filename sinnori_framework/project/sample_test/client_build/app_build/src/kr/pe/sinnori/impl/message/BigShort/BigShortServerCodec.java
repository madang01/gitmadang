...
public final class BigShortServerCodec implements MessageCodecIF {
	...
	public MessageDecoder getMessageDecoder() throws DynamicClassCallException {
		throw new DynamicClassCallException("BigShort메시지는 클라이언트에서 서버로 전달하지 않는 메시지 입니다.");
	}

	@Override
	public MessageEncoder getMessageEncoder() throws DynamicClassCallException {
		throw new DynamicClassCallException("BigShort 메시지는 서버에서 클라이언트로 전달하지 않는 메시지 입니다.");
	}
}