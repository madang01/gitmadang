	...
	protected void performTask(HttpServletRequest req, HttpServletResponse res)
		...
		if (null == pageGubun || pageGubun.equals("step1")) {
			...
			synchronized (session) {
				...
				String projectName = System.getProperty(CommonStaticFinalVars.SINNORI_CONFIG_FILE_JAVA_SYSTEM_VAR_NAME);		
				...
				if (errorMessage.equals("")) {
					...
					memberSessionKeyInObj.setIdCipherBase64(servletSymmetricKey.encryptStringBase64(id));
					memberSessionKeyInObj.setPwdCipherBase64(servletSymmetricKey.encryptStringBase64(pwd));
					memberSessionKeyInObj.setNicknameCipherBase64(servletSymmetricKey.encryptStringBase64(nickname));
					memberSessionKeyInObj.setQuestionCipherBase64(servletSymmetricKey.encryptStringBase64(question));
					memberSessionKeyInObj.setAnswerCipherBase64(servletSymmetricKey.encryptStringBase64(answer));
					...
					AbstractMessage messageFromServer = clientProject.sendSyncInputMessage(memberSessionKeyInObj);					
					if (messageFromServer instanceof MessageResult) {
						MessageResult outObj = (MessageResult)messageFromServer;
						
						req.setAttribute("resultOutputMessage", outObj);
					} else {
						errorMessage = messageFromServer.toString();
						log.warn(errorMessage);
					}		
