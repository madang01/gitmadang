...
<li>
	<dl>
		<dt><h2>신놀이 프레임워크란?</h2></dt>
			<dd><h3> "신놀이" 이름은 "신명나게 놀아 보자" 라는 뜻입니다.<br/>
	...
	<dl>
		<dt><h3>신놀이 서버</h3></dt>
		<dd><h3>한빛미디어 김성박/송지훈님의 "자바 I/O & NIO 네트워크 프로그래밍" 에서 소개한 AdvancedChatServer 기반으로 만들었습니다.</h3></dd>
	</dl>
	</li>
	<li>
	<dl>
		<dt><h3>클라이언트 서버 접속 라이브러리</h3></dt>
		<dd><h3>신놀이 서버에 연결하여 메세지를 주고 받을 수 있는 라이브러리로 구성된다.<br/>
		2013년 현재 자바 신놀이 클라이언트 라이브러리만 제공된다.<br/>
		연결 폴의 종류는 3가지이며 그중 하나는 다른 사용자와 소캣 자원을 공유하는 방법을 제공하는것이 특색이다.</h3></dd>
	</dl>
		<ol>
			<li><dl>
		<dt><h3>자바 신놀이 클라이언트 라이브러리</h3></dt>
		<dd><h3>자바언어로 만든 비동기 통신방식으로 신놀이 서버와 연결하는 신놀이 클라이언트 라이브러리이다.</h3></dd>
	</dl></li>
		</ol>
	</li>
	</ol>
...
<li>
	<dl>
		<dt><h2>개발한것을 아파치 라이센스 2.0 으로 공개하는 특별한 이유?</h2></dt>
		<dd><h3>개인적으로 자유 소프트웨어 진영에 공헌을 하고 싶고<br/>
...
<li>
	<dl>
		<dt><h2>개발을 하게된 동기및 목표</h2></dt>
		<dd><h3>신놀이 프레임 워크 개발 동기는 윷놀이 서버/클라이언트 게임을 만들고자 시작되었습니다. <br/>
		...
		신놀이는 밑그림 그려진 종이 + 크레용  라는 점입니다.<br/>
		...
		tcp/ip server/client 응용 프로그램을 만들고 싶은 분들한테도 <br/>
		저는 이런 즉각적인 확인과 편의성을 제공해 드리고 싶었습니다.<br/>		
		서버 개발자 입장에서 클라이언트 개발자랑 의논해서 IO 를 정의한후<br/>
		IO 에 맞추어서 입력/출력 메시지를 만들거나 혹은<br/>
		클라이언트 개발자가 만든 입력/출력 메시지를 공유한후<br/>
		입력 메시지 1:1 대응하는 서버 비지니스 로직만 작성하면 되고,<br/>
		클라이언트 개발자 입장에서는 서버 개발자랑 의논하여 IO 를 정의한후<br/>
		IO 에 맞추어서 신놀이 입력/출력메시지를 만들거나 <br/>
		혹은 서버 개발자가만든것을 공유한후 <br/>
		입력 메시지를 서버로 보내어 출력 메시지를 얻어 클라이언트 로직을 작성하면 됩니다.</h3>
		<h2>이것이 제가 생각하는 개발 방법론이며 신놀이가 지향하는 방향으로<br/>
		2000년 윷놀이 게임 ERD를 완성후 제가 찾았던 공개용 개발 프레임워크입니다.</h2>
		</dd>
	</dl>
</li>
</ol>
