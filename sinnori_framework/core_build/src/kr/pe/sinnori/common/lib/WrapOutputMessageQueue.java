 ...
 *
 */
public class WrapOutputMessageQueue implements CommonRootIF {
	private boolean isInQueue = true;
	private LinkedBlockingQueue<OutputMessage> outputMessageQueue = null;
	
	public WrapOutputMessageQueue(LinkedBlockingQueue<OutputMessage> outputMessageQueue) {
	...
	public void queueIn() {
		...
		while (!outputMessageQueue.isEmpty()) {
			...
			log.warn(String.format("랩 출력 메시지큐를 큐에 반환하므로 가지고 있던 출력 메시지[%d][%s] 삭제", i++, outObj.toString()));
		}
	}
	 ...
	 */
	public LinkedBlockingQueue<OutputMessage> getOutputMessageQueue() {
		return outputMessageQueue;
	}

	@Override
	public String toString() {
		StringBuilder strBuffer = new StringBuilder("WrapOutputMessageQueue=[");
