...
CREATE SCHEMA IF NOT EXISTS `LT_SB_DB` DEFAULT CHARACTER SET utf8 ;

CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_BOARD_INFO_TB` (
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_MEMBER_TB` (
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_BOARD_TB` (
  ...
  CONSTRAINT `sb_board_fk1`
    ...
    REFERENCES `LT_SB_DB`.`SB_BOARD_INFO_TB` (`board_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_BOARD_FILELIST_TB` (
  ...
  CONSTRAINT `sb_board_filelist_fk1`
    ...
    REFERENCES `LT_SB_DB`.`SB_BOARD_TB` (`board_id` , `board_no`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_BOARD_VOTE_TB` (
  `board_id` TINYINT(3) UNSIGNED NOT NULL,
  `board_no` INT(10) UNSIGNED NOT NULL,
  ...
  CONSTRAINT `sb_board_vote_fk2`
    FOREIGN KEY (`user_id`)
    REFERENCES `LT_SB_DB`.`SB_MEMBER_TB` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_SEQ_TB` (
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_SITEMENU_TB` (
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_BOARD_HISTORY_TB` (
  `board_id` TINYINT(3) UNSIGNED NOT NULL,
  `board_no` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_SITE_LOG_TB` (
...
CREATE TABLE IF NOT EXISTS `LT_SB_DB`.`SB_MEMBER_ACTIVITY_HISTORY_TB` (
