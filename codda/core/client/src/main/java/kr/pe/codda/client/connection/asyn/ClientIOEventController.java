...
public class ClientIOEventController extends Thread implements ClientIOEventControllerIF {
	private InternalLogger log = InternalLoggerFactory.getInstance(ClientIOEventController.class);
	...
	private ConcurrentHashMap<SelectionKey, ClientIOEventHandlerIF> selectedKey2ConnectionHash = 
			new ConcurrentHashMap<SelectionKey, ClientIOEventHandlerIF>();
	...
	public ClientIOEventController(long clientSelectorWakeupInterval, AsynConnectionPoolIF connectionPool) throws IOException, NoMoreDataPacketBufferException {
	...
	@Override
	public void addUnregisteredAsynConnection(ClientIOEventHandlerIF unregisteredAsynConnection) {
	...
	private void processNewConnection() {					
		while (! unregisteredAsynConnectionQueue.isEmpty()) {
			ClientIOEventHandlerIF unregisteredAsynConnection = unregisteredAsynConnectionQueue.removeFirst();
			...
			while (! isInterrupted()) {
				...
				for (SelectionKey selectedKey : selectedKeySet) {
					...
					if (selectedKey.isConnectable()) {
						ClientIOEventHandlerIF  interestedAsynConnection = selectedKey2ConnectionHash.get(selectedKey);
