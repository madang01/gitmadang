...
public class AsynNoShareConnection implements AsynConnectionIF, ClientIOEventHandlerIF, ReceivedMessageReceiverIF {
	...
	private void addInputMessage(MessageCodecMangerIF messageCodecManger, AbstractMessage inputMessage)
		...
		synchronized (writeMonitor) {
			...
			*/
			...
			if (inputMessageQueue.size() >= clientAsynInputMessageQueueCapacity) {
				...
				if (inputMessageQueue.size() >= clientAsynInputMessageQueueCapacity) {
					String errorMessage = new StringBuilder().append("fail to inserts the specified element[")
							.append(inputMessage.toString()).append("] into the input message queue while connection[")
					...
					while (!wrapBufferList.isEmpty()) {
						WrapBuffer wrapBuffer = wrapBufferList.removeFirst();
						dataPacketBufferPool.putDataPacketBuffer(wrapBuffer);
					}
			...
			inputMessageQueue.addLast(wrapBufferList);
