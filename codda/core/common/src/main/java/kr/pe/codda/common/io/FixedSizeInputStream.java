...
public class FixedSizeInputStream implements BinaryInputStreamIF {
	...
	private String doGetString(int length, Charset stringCharset) throws BufferUnderflowExceptionWithMessage, CharsetDecoderException {
	...
	private void throwExceptionIfNumberOfBytesRemainingIsLessThanNumberOfBytesRequired(int numberOfBytesRequired)
		...
		if (numberOfBytesRemaining < numberOfBytesRequired) {
			throw new BufferUnderflowExceptionWithMessage(
	...
	public byte getByte() throws BufferUnderflowExceptionWithMessage {
		...
		if (0 == remainingBytes) {
			throw new BufferUnderflowExceptionWithMessage("the remaining bytes is zero");
		}
	...
	public short getShort() throws BufferUnderflowExceptionWithMessage {
		long remainingBytes = available();
		if (2 > remainingBytes) {
			throw new BufferUnderflowExceptionWithMessage(String.format("the remaining bytes[%d] is less than two bytes", remainingBytes));
		}
	...
	public int getInt() throws BufferUnderflowExceptionWithMessage {
		long remainingBytes = available();
		if (4 > remainingBytes) {
			throw new BufferUnderflowExceptionWithMessage(String.format("the remaining bytes[%d] is less than four bytes", remainingBytes));
		}
	...
	public long getLong() throws BufferUnderflowExceptionWithMessage {
		long remainingBytes = available();
		if (8 > remainingBytes) {
			throw new BufferUnderflowExceptionWithMessage(String.format("the remaining bytes[%d] is less than eight bytes", remainingBytes));
		}
	...
	public void getBytes(byte[] dst, int offset, int length)
		...
		if (length > remainingBytes) {
			throw new BufferUnderflowExceptionWithMessage(String.format("the parameter length[%d] is greater than the remaining bytes[%d]", length, remainingBytes));
		}
	...
	@Override
	public byte[] getBytes(int length) throws BufferUnderflowExceptionWithMessage,
	...
	public void skip(int n) throws BufferUnderflowExceptionWithMessage {
		...
		long remainingBytes = available();
		if (n > remainingBytes) {
			throw new BufferUnderflowExceptionWithMessage(String.format("the parameter n[%d] is greater than the remaining bytes[%d]", n, remainingBytes));
		}
