...
public abstract class AbstractMinMaxConverter<T extends Number> 
	extends AbstractNativeValueConverter<T> {
	
	protected T min;
	protected T max;
	private Comparator<T> typeComparator = null;

	public AbstractMinMaxConverter(T min, T max, 
			Comparator<T> typeComparator, Class<T> genericType) {
		super(genericType);
		
		if (null == min) {
			String errorMessage = new StringBuilder("parameter min is null")
					.toString();
			throw new IllegalArgumentException(errorMessage);
		}

		if (null == max) {
			String errorMessage = new StringBuilder("parameter max is null")
					.toString();
			throw new IllegalArgumentException(errorMessage);
		}
		
		if (null == typeComparator) {
			String errorMessage = new StringBuilder("parameter typeComparator is null")
					.toString();
			throw new IllegalArgumentException(errorMessage);
		}
	...
	/**
	 * {@link #valueOf(String)} 에서 호출되는 내부용 메소드로
	 * {@link #valueOf(String)} 에서 최소 최대값 제약이라는 공통 부분을 제외한
	 * 지정한 타입으로 입력받은 문자열 타입의 항목의 값을 변환하는 기능을 담당한다.
	 * 
