...
public class ProjectBuilderTest extends AbstractJunitTest {
	...
	/*public void testGetNewSinnoriConfigSequencedProperties() {
		String installedPathString = "D:\\gitsinnori\\sinnori2";
		...
		SequencedProperties newSinnoriConfigSequencedProperties = mainProjectItemIDInfo
				.getNewSinnoriConfigSequencedProperties(installedPathString, mainProjectName);
	...
	public void testConstructor_badSinnoriInstalledPath_notExist() {
		...
		try {
			...
			if (!errorMessage.equals(
					String.format("the Sinnori installed path[%s] does not exist", installedPathString))) {
	...
	public void testConstructor_badSinnoriInstalledPath_notDirecotry() {
		...
		try {			
			...
			String errorMessage = e.getMessage();
			if (!errorMessage.equals(
					String.format("the Sinnori installed path[%s] isn't a directory", installedPathString))) {
				log.warn(e.getMessage(), e);
				fail(e.getMessage());
			}
		}
	...
	public void testConstructor_badProjectBasePath_notExist() {
		...
		try {
			...
			if (!errorMessage.equals(String.format("the project base path[%s] does not exist",
					BuildSystemPathSupporter.getProjectBasePathString(installedPathString)))) {
				log.warn(e.getMessage(), e);
				fail(e.getMessage());
			}
		}
	...
	public void testConstructor_badProjectBasePath_notDirectory() {
		...
		boolean resultCreatingSinnoriInstalledPathForTest = installedPathForTest.mkdir();
		...
		try {
			...
			if (!errorMessage.equals(String.format("the project base path[%s] isn't a directory",
					BuildSystemPathSupporter.getProjectBasePathString(installedPathStringForTest)))) {
				log.warn(e.getMessage(), e);
				fail(e.getMessage());
			}
		}
	...
	public void testConstructor_badProjectName_notDirectory() {
		...
		File installedPathForTest = new File(installedPathStringForTest);
		boolean resultCreatingSinnoriInstalledPathForTest = installedPathForTest.mkdirs();
		if (!resultCreatingSinnoriInstalledPathForTest) {
			fail("fail to create Sinnori installed path For Test");
		}
		installedPathForTest.deleteOnExit();
		...
		String projectPathString = BuildSystemPathSupporter.getProjectPathString(installedPathStringForTest,
				badMainProjectName);
	...
	private void checkAntBuildForServer(String installedPathString, String mainProjectName) {
		...
		String severBuildPathString = BuildSystemPathSupporter.getServerBuildPathString(installedPathString,
				mainProjectName);
	...
	public void testApplySinnoriInstalledPath() {
		...
		try {
			...
			String projectResourcesDirectoryPathString = BuildSystemPathSupporter.getProjectResourcesDirectoryPathString(installedPathString, mainProjectName);
