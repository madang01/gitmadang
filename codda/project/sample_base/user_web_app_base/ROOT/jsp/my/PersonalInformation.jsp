...
%><%@ page import="kr.pe.codda.impl.message.PersonalInformationRes.PersonalInformationRes" %><%
...
%><jsp:useBean id="personalInformationRes" class="kr.pe.codda.impl.message.PersonalInformationRes.PersonalInformationRes" scope="request" /><%
...
<div class="content">
	<div class="container">
		<div class="panel panel-default">
			<div class="panel-heading"><h4>개인 정보 조회</h4></div>
			<div class="panel-body">
				<table class="table">
					<tbody><% 
	if (null != personalInformationRes.getTargetUserID() && ! personalInformationRes.getTargetUserID().isEmpty()) {
		MemberRoleType memberRoleType = MemberRoleType.valueOf(personalInformationRes.getRole());
		MemberStateType memberStateType = MemberStateType.valueOf(personalInformationRes.getState());
		
		FastDateFormat sdf = FastDateFormat.getInstance( "yyyy년 MM월 dd일 HH시 mm분 ss초");
		
%>
						<tr><td>아이디</td><td><%= personalInformationRes.getTargetUserID() %></td></tr>
						<tr><td>별명</td><td><%= personalInformationRes.getNickname() %></td></tr>
						<tr><td>상태</td><td><%= memberStateType.getName() %></td></tr>
						<tr><td>역활</td><td><%= memberRoleType.getName() %></td></tr>
						<tr><td>최초 가입일</td><td><%= sdf.format(personalInformationRes.getRegisteredDate()) %></td></tr>
						<tr><td>마지막 수정일</td><td><%= sdf.format(personalInformationRes.getModifiedDate()) %></td></tr><%
	} else {
		out.write("<tr><td><p>개인 정보 조회는 지정 받은 사용자에 대해서 개인정보 조회를 수행합니다. 단 지정된 대상이 없다면 안내 문구만 소개하지만 로그인 했을 경우 로그인 사용자가 지정된 사용자로 대체됩니다.</p></td><td></tr>");
	}
%>
					</tbody>
				</table>
