...
/**
 * MemberManagerRes message encoder
 * @author Won Jonghoon
 *
 */
public final class MemberManagerResEncoder extends AbstractMessageEncoder {
	@Override
	public void encode(AbstractMessage messageObj, SingleItemEncoderIF singleItemEncoder, Object writableMiddleObject) throws Exception {
		MemberManagerRes memberManagerRes = (MemberManagerRes)messageObj;
		encodeBody(memberManagerRes, singleItemEncoder, writableMiddleObject);
	}


	private void encodeBody(MemberManagerRes memberManagerRes, SingleItemEncoderIF singleItemEncoder, Object middleWritableObject) throws Exception {
		java.util.LinkedList<String> pathStack = new java.util.LinkedList<String>();
		pathStack.push("MemberManagerRes");


		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "memberState"
			, kr.pe.codda.common.type.SingleItemType.BYTE // itemType
			, memberManagerRes.getMemberState() // itemValue
			, -1 // itemSize
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "searchID"
			...
			, memberManagerRes.getSearchID() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "fromDateString"
			, kr.pe.codda.common.type.SingleItemType.UB_PASCAL_STRING // itemType
			, memberManagerRes.getFromDateString() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "toDateString"
			, kr.pe.codda.common.type.SingleItemType.UB_PASCAL_STRING // itemType
			, memberManagerRes.getToDateString() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "pageNo"
			...
			, memberManagerRes.getPageNo() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "pageSize"
			...
			, memberManagerRes.getPageSize() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "isNextPage"
			...
			, memberManagerRes.getIsNextPage() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "cnt"
			, kr.pe.codda.common.type.SingleItemType.INTEGER // itemType
			, memberManagerRes.getCnt() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);

		java.util.List<MemberManagerRes.Member> member$2List = memberManagerRes.getMemberList();

		/** 배열 정보와 배열 크기 일치 검사 */
		if (null == member$2List) {
			/** 배열 크기 지정 방식이 간접일 경우 참조하는 변수값이 0 일 경우만 배열 값으로 null 을 허용한다. */
			if (0 != memberManagerRes.getCnt()) {
				String errorMessage = new StringBuilder("the var member$2List is null but the value referenced by the array size[memberManagerRes.getCnt()][").append(memberManagerRes.getCnt()).append("] is not zero").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
		} else {
			int member$2ListSize = member$2List.size();
			/** 배열 값이 null 이 아닐때에는 배열 크기가 배열 정보에서 지정된 크기와 같은지 검사 */
			if (memberManagerRes.getCnt() != member$2ListSize) {
				String errorMessage = new StringBuilder("the var member$2ListSize[").append(member$2ListSize).append("] is not same to the value referenced by the array size[memberManagerRes.getCnt()][").append(memberManagerRes.getCnt()).append("]").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
			...
			for (int i2=0; i2 < member$2ListSize; i2++) {
				...
				MemberManagerRes.Member member$2 = member$2List.get(i2);
