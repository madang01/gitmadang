...
/**
 * ArraySiteMenuRes message encoder
 * @author Won Jonghoon
 *
 */
public final class ArraySiteMenuResEncoder extends AbstractMessageEncoder {
	...
	private void encodeBody(ArraySiteMenuRes arraySiteMenuRes, SingleItemEncoderIF singleItemEncoder, Object middleWritableObject) throws Exception {
		...
		if (null == menu$2List) {
			...
			if (0 != arraySiteMenuRes.getCnt()) {
				String errorMessage = new StringBuilder("the var menu$2List is null but the value referenced by the array size[arraySiteMenuRes.getCnt()][").append(arraySiteMenuRes.getCnt()).append("] is not zero").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
		} else {
			int menu$2ListSize = menu$2List.size();
			/** 배열 값이 null 이 아닐때에는 배열 크기가 배열 정보에서 지정된 크기와 같은지 검사 */
			if (arraySiteMenuRes.getCnt() != menu$2ListSize) {
				String errorMessage = new StringBuilder("the var menu$2ListSize[").append(menu$2ListSize).append("] is not same to the value referenced by the array size[arraySiteMenuRes.getCnt()][").append(arraySiteMenuRes.getCnt()).append("]").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
