...
public class UserLoginProcessSvl extends AbstractServlet {
	...
	/*
	 * private void printUserLoginFailureCallBackPage(HttpServletRequest req,
	 * HttpServletResponse res, ServerSymmetricKeyIF webServerSymmetricKey,
	 * String modulusHexString, String errorMessage) {
	 * req.setAttribute(WebCommonStaticFinalVars
	 * .REQUEST_KEY_NAME_OF_WEB_SERVER_SYMMETRIC_KEY, webServerSymmetricKey);
	 * req
	 * .setAttribute(WebCommonStaticFinalVars.REQUEST_KEY_NAME_OF_MODULUS_HEX_STRING
	 * , modulusHexString); req.setAttribute("errorMessage", errorMessage);
	 * 
	...
	protected void performTask(HttpServletRequest req, HttpServletResponse res)
		 ...
		 * debugMessage = errorMessage; printErrorMessagePage(req, res,
		 * errorMessage, debugMessage); return; }
		 */
		...
		synchronized (req) {
			clientSessionKey = (ClientSessionKeyIF) req
					.getAttribute(WebCommonStaticFinalVars.HTTPSESSION_KEY_NAME_OF_CLIENT_SESSIONKEY);
			...
			if (null == clientSessionKey) {
				...
				AbstractMessage binaryPublicKeyOutputMessage = mainProjectConnectionPool
						.sendSyncInputMessage(
								ClientMessageCodecManger.getInstance(),
								binaryPublicKeyReq);
				...
				byte[] binaryPublicKeyBytes = binaryPublicKeyRes
						.getPublicKeyBytes();
				...
				req.setAttribute(
						WebCommonStaticFinalVars.HTTPSESSION_KEY_NAME_OF_CLIENT_SESSIONKEY,
						clientSessionKey);
			}
		}
		...
		try {
			memberRoleType = MemberRoleType.valueOf(
					userLoginRes.getMemberRole(), false);
		} catch (IllegalArgumentException e) {
			log.warn("사용자[{}]의 멤버 종류[{}] 가 잘못되어 멤버 종류를 '일반 유저'로 강제 변경합니다",
					userLoginRes.getUserID(), userLoginRes.getMemberRole());
		}
