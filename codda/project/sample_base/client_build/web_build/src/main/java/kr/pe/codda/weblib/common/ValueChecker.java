...
public class ValueChecker {
	...
	/**
	 * 페이지 번호 검사, 0 <= 페이지 번호 <= unsigned short 최대값(=65535)
	 * @param paramPageNo
	 * @return
	 * @throws IllegalArgumentException
	 */
	public static int checkValidPageNo(String paramPageNo) throws IllegalArgumentException {
		int pageNo = 1;
		if (null != paramPageNo) {
			try {
			...
			if (pageNo <= 0) {
				String errorMessage = new StringBuilder(" 페이지 번호[")
						.append(paramPageNo).append("]가 0보다 작거나 같습니다").toString();
				throw new IllegalArgumentException(errorMessage);
			}
			
			if (pageNo > CommonStaticFinalVars.UNSIGNED_SHORT_MAX) {
				String errorMessage = new StringBuilder("페이지 번호[")
						.append(paramPageNo).append("]가  unsigned short 최대값 65535 를 넘었습니다").toString();
				throw new IllegalArgumentException(errorMessage);
			}
		}
		return pageNo;
