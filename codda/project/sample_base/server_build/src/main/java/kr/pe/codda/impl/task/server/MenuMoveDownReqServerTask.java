...
public class MenuMoveDownReqServerTask extends AbstractServerTask {	
	...
	public MessageResultRes doWork(String dbcpName, MenuMoveDownReq menuMoveDownReq) throws Exception {
		...
		try {
			...
			String memberRoleOfRequestedUserID = ValueChecker.checkValidRequestedUserState(conn, create, log, menuMoveDownReq.getRequestedUserID());
			...
			try {
				...
				String errorMessage = new StringBuilder("알 수 없는 회원[")
					.append(menuMoveDownReq.getRequestedUserID())
					.append("]의 역활[")
			...
			if (! MemberRoleType.ADMIN.equals(memberRoleTypeOfRequestedUserID)) {
				...
				String errorMessage = "메뉴 하단 이동 서비스는 관리자 전용 서비스입니다";
				throw new ServerServiceException(errorMessage);
			}
