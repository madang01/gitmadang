...
/**
 * MenuListRes message encoder
 * @author Won Jonghoon
 *
 */
public final class MenuListResEncoder extends AbstractMessageEncoder {
	@Override
	public void encode(AbstractMessage messageObj, SingleItemEncoderIF singleItemEncoder, Object writableMiddleObject) throws Exception {
		MenuListRes menuListRes = (MenuListRes)messageObj;
		encodeBody(menuListRes, singleItemEncoder, writableMiddleObject);
	}


	private void encodeBody(MenuListRes menuListRes, SingleItemEncoderIF singleItemEncoder, Object middleWritableObject) throws Exception {
		...
		singleItemEncoder.putValueToWritableMiddleObject(pathStack.peek(), "cnt"
			...
			, menuListRes.getCnt() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleWritableObject);

		java.util.List<MenuListRes.Menu> menu$2List = menuListRes.getMenuList();

		/** 배열 정보와 배열 크기 일치 검사 */
		if (null == menu$2List) {
			/** 배열 크기 지정 방식이 간접일 경우 참조하는 변수값이 0 일 경우만 배열 값으로 null 을 허용한다. */
			if (0 != menuListRes.getCnt()) {
				String errorMessage = new StringBuilder("the var menu$2List is null but the value referenced by the array size[menuListRes.getCnt()][").append(menuListRes.getCnt()).append("] is not zero").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
		} else {
			int menu$2ListSize = menu$2List.size();
			/** 배열 값이 null 이 아닐때에는 배열 크기가 배열 정보에서 지정된 크기와 같은지 검사 */
			if (menuListRes.getCnt() != menu$2ListSize) {
				String errorMessage = new StringBuilder("the var menu$2ListSize[").append(menu$2ListSize).append("] is not same to the value referenced by the array size[menuListRes.getCnt()][").append(menuListRes.getCnt()).append("]").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
			...
			for (int i2=0; i2 < menu$2ListSize; i2++) {
				...
				MenuListRes.Menu menu$2 = menu$2List.get(i2);
