/*
 * This file is generated by jOOQ.
*/
package kr.pe.codda.impl.jooq.tables.records;


import javax.annotation.Generated;

import kr.pe.codda.impl.jooq.tables.SbBoardInfoTb;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record9;
import org.jooq.Row9;
import org.jooq.impl.UpdatableRecordImpl;
import org.jooq.types.UByte;
import org.jooq.types.UInteger;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.10.6"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class SbBoardInfoTbRecord extends UpdatableRecordImpl<SbBoardInfoTbRecord> implements Record9<UByte, String, Byte, Byte, Byte, Byte, Long, Long, UInteger> {

    private static final long serialVersionUID = -811901966;

    /**
     * Setter for <code>sb_db.sb_board_info_tb.board_id</code>. 게시판 식별자,
1 : 공지, 2:자유, 3:이슈
     */
    public void setBoardId(UByte value) {
        set(0, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.board_id</code>. 게시판 식별자,
1 : 공지, 2:자유, 3:이슈
     */
    public UByte getBoardId() {
        return (UByte) get(0);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.board_name</code>. 게시판 이름
     */
    public void setBoardName(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.board_name</code>. 게시판 이름
     */
    public String getBoardName() {
        return (String) get(1);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.list_type</code>. 게시판 목록 유형, 0:그룹 루트, 1; 계층
     */
    public void setListType(Byte value) {
        set(2, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.list_type</code>. 게시판 목록 유형, 0:그룹 루트, 1; 계층
     */
    public Byte getListType() {
        return (Byte) get(2);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.reply_policy_type</code>. 댓글 정책 유형, 0:댓글없음, 1:본문글에만, 2:본문및 댓글 모두
     */
    public void setReplyPolicyType(Byte value) {
        set(3, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.reply_policy_type</code>. 댓글 정책 유형, 0:댓글없음, 1:본문글에만, 2:본문및 댓글 모두
     */
    public Byte getReplyPolicyType() {
        return (Byte) get(3);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.write_permission_type</code>. 본문 쓰기 권한 유형, 0:어드민, 1:일반인, 2:손님
     */
    public void setWritePermissionType(Byte value) {
        set(4, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.write_permission_type</code>. 본문 쓰기 권한 유형, 0:어드민, 1:일반인, 2:손님
     */
    public Byte getWritePermissionType() {
        return (Byte) get(4);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.reply_permission_type</code>. 댓글 쓰기 권한 유형, 0:어드민, 1:일반인, 2:손님, 주) '댓글 쓰기 권한 유형'은 '상세 유형'에서 '0 댓글없음' 이 아닌 경우만 유효하다
     */
    public void setReplyPermissionType(Byte value) {
        set(5, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.reply_permission_type</code>. 댓글 쓰기 권한 유형, 0:어드민, 1:일반인, 2:손님, 주) '댓글 쓰기 권한 유형'은 '상세 유형'에서 '0 댓글없음' 이 아닌 경우만 유효하다
     */
    public Byte getReplyPermissionType() {
        return (Byte) get(5);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.cnt</code>. 게시판 목록 개수
     */
    public void setCnt(Long value) {
        set(6, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.cnt</code>. 게시판 목록 개수
     */
    public Long getCnt() {
        return (Long) get(6);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.total</code>. 게시판 글 전체 개수
     */
    public void setTotal(Long value) {
        set(7, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.total</code>. 게시판 글 전체 개수
     */
    public Long getTotal() {
        return (Long) get(7);
    }

    /**
     * Setter for <code>sb_db.sb_board_info_tb.next_board_no</code>. 다음 게시판 번호, 본문의 부모 게시판 번호가 0 으로 예약되어 있어 1부터 시작
     */
    public void setNextBoardNo(UInteger value) {
        set(8, value);
    }

    /**
     * Getter for <code>sb_db.sb_board_info_tb.next_board_no</code>. 다음 게시판 번호, 본문의 부모 게시판 번호가 0 으로 예약되어 있어 1부터 시작
     */
    public UInteger getNextBoardNo() {
        return (UInteger) get(8);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Record1<UByte> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record9 type implementation
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Row9<UByte, String, Byte, Byte, Byte, Byte, Long, Long, UInteger> fieldsRow() {
        return (Row9) super.fieldsRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Row9<UByte, String, Byte, Byte, Byte, Byte, Long, Long, UInteger> valuesRow() {
        return (Row9) super.valuesRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<UByte> field1() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.BOARD_ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field2() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.BOARD_NAME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Byte> field3() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.LIST_TYPE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Byte> field4() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.REPLY_POLICY_TYPE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Byte> field5() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.WRITE_PERMISSION_TYPE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Byte> field6() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.REPLY_PERMISSION_TYPE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Long> field7() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.CNT;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Long> field8() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.TOTAL;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<UInteger> field9() {
        return SbBoardInfoTb.SB_BOARD_INFO_TB.NEXT_BOARD_NO;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UByte component1() {
        return getBoardId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component2() {
        return getBoardName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte component3() {
        return getListType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte component4() {
        return getReplyPolicyType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte component5() {
        return getWritePermissionType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte component6() {
        return getReplyPermissionType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long component7() {
        return getCnt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long component8() {
        return getTotal();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UInteger component9() {
        return getNextBoardNo();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UByte value1() {
        return getBoardId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value2() {
        return getBoardName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte value3() {
        return getListType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte value4() {
        return getReplyPolicyType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte value5() {
        return getWritePermissionType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Byte value6() {
        return getReplyPermissionType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long value7() {
        return getCnt();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long value8() {
        return getTotal();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UInteger value9() {
        return getNextBoardNo();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value1(UByte value) {
        setBoardId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value2(String value) {
        setBoardName(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value3(Byte value) {
        setListType(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value4(Byte value) {
        setReplyPolicyType(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value5(Byte value) {
        setWritePermissionType(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value6(Byte value) {
        setReplyPermissionType(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value7(Long value) {
        setCnt(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value8(Long value) {
        setTotal(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord value9(UInteger value) {
        setNextBoardNo(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public SbBoardInfoTbRecord values(UByte value1, String value2, Byte value3, Byte value4, Byte value5, Byte value6, Long value7, Long value8, UInteger value9) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached SbBoardInfoTbRecord
     */
    public SbBoardInfoTbRecord() {
        super(SbBoardInfoTb.SB_BOARD_INFO_TB);
    }

    /**
     * Create a detached, initialised SbBoardInfoTbRecord
     */
    public SbBoardInfoTbRecord(UByte boardId, String boardName, Byte listType, Byte replyPolicyType, Byte writePermissionType, Byte replyPermissionType, Long cnt, Long total, UInteger nextBoardNo) {
        super(SbBoardInfoTb.SB_BOARD_INFO_TB);

        set(0, boardId);
        set(1, boardName);
        set(2, listType);
        set(3, replyPolicyType);
        set(4, writePermissionType);
        set(5, replyPermissionType);
        set(6, cnt);
        set(7, total);
        set(8, nextBoardNo);
    }
}
