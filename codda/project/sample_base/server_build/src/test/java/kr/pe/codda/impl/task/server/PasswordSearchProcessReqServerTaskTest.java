...
public class PasswordSearchProcessReqServerTaskTest extends AbstractJunitTest {
	...
	public void 비밀번호찾기처리_비밀값틀린횟수최대값초과() {
		...
		try {
			...
			create.delete(SB_ACCOUNT_SERARCH_REQ_TB)
			.where(SB_ACCOUNT_SERARCH_REQ_TB.USER_ID.eq(testID))
			.execute();
		...
		try {
			...
			String errorMessage = e.getMessage();
			String expectedErrorMessage = new StringBuilder().append("아이디 찾기로 비밀번호 틀린 횟수가  최대 횟수 ")
	...
	public void 비밀번호찾기처리_시간초과() {
		...
		try {
			...
			String errorMessage = e.getMessage();
			String expectedErrorMessage = new StringBuilder().append("아이디 찾기에서 비밀번호 입력 제한 시간[")
	...
	public void 비밀번호찾기처리_OK() {
		...
		try {
			...
			Record1<String> passwordSearchReqRecord = create.select(SB_ACCOUNT_SERARCH_REQ_TB.LAST_SECRET_AUTH_VALUE)
			.from(SB_ACCOUNT_SERARCH_REQ_TB)
			.where(SB_ACCOUNT_SERARCH_REQ_TB.USER_ID.eq(testID))
			.fetchOne();
			
			secretAuthenticationValue = passwordSearchReqRecord.get(SB_ACCOUNT_SERARCH_REQ_TB.LAST_SECRET_AUTH_VALUE);
