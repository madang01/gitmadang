...
public class MemberManagerReqServerTaskTest extends AbstractJunitTest {
	...
	public void testDoWork() {
		...
		memberManagerReq.setRequestedUserID("admin");
		memberManagerReq.setMemberState(MemberStateType.OK.getValue());
		memberManagerReq.setSearchID("");
		memberManagerReq.setFromDateString(sdf.format(fromDate));
		memberManagerReq.setToDateString(sdf.format(toDate));
		memberManagerReq.setPageNo(1);
		memberManagerReq.setPageSize(20);
		
		log.info(memberManagerReq.toString());
		
		MemberManagerReqServerTask memberManagerReqServerTask = null;
		
		try {
			memberManagerReqServerTask = new MemberManagerReqServerTask();
		} catch(DynamicClassCallException e) {
			log.warn("회원 관리자 타스크 동적 클래스 호출 실패", e);
			fail("dead code, 회원 관리자 타스크 동적 클래스 호출 실패");
		}
		
		MemberManagerRes memberManagerRes = null;
		
		try {
			memberManagerRes = memberManagerReqServerTask.doWork(TEST_DBCP_NAME, memberManagerReq);
		} catch (Exception e) {
			log.warn("회원 관리자 타스크 실행 실패", e);
			fail("회원 관리자 타스크 실행 실패::errmsg="+e.getMessage());
		}
		
		log.info(memberManagerRes.toString());
	}
