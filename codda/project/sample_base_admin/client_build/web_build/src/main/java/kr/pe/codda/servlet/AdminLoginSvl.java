...
public class AdminLoginSvl extends AbstractServlet {
	...
	private void loginResultPage(HttpServletRequest req, HttpServletResponse res)
		...
		String parmIdCipherBase64 = req.getParameter("id");
		...
		if (null == parmIdCipherBase64) {
			String errorMessage = "the request parameter parmIdCipherBase64 is null";
			String debugMessage = errorMessage;
			printErrorMessagePage(req, res, errorMessage, debugMessage);
			return;
		}
		
		if (! Base64.isBase64(parmIdCipherBase64)) {
			String errorMessage = "the request parameter parmIdCipherBase64 is not a base64 cipher text";
			String debugMessage = errorMessage;
			printErrorMessagePage(req, res, errorMessage, debugMessage);
			return;
		}
		...
		log.info("parm id=[{}]", parmIdCipherBase64);
		...
		byte[] idBytes = webServerSymmetricKey.decrypt(Base64.decodeBase64(parmIdCipherBase64));
		...
		// FIXME!
		log.info("id=[{}]", new String(idBytes, "UTF8"));
